...
<div id="ArticleList">
  <div class="article-module d-flex br-4 mb-30 fade-in" v-for="article in articles" :key="article.id">
    ...
    <div class="align-self-top pl-20">
        <div class="votes text-center">
          <!-- <p class="c-white">{{article.statusType}}</p> -->
          <b-button class="vote btn btn-transparent bg-brand icon-wrap" @click="upVote(article.id, article.vote_count)">
            <svg v-if="article.statusType.id == '70'" id="ten-x" class="push-button" width="34" height="42" viewBox="0 0 22 28" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  <div class="c-white" v-if="isLoading">Loading...</div>
</div>
</template>
...
export default {
	...
	methods: {
		...
		loadArticles(page = this.pageno, limit = this.limit) {
			this.isLoading = true;
			const _self = this;
			axios.get("http://bitcoinersbest.local:9111/v1/items?access-token=admin-bandit-authkey&V1ResItemSearch[res_type_id]=30&page=" + page + "&limit=" + limit).then(response => {
